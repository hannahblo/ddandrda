...
test_ufg_classic <- function(observed,
                             observed_in_grid,
                             sf_spatial,
                             grid_spatial,
                             grid_numeric,
  ...
  hull_set <- compute_hull_inner(observed[, c(1,2)],
                                 observed_in_grid,
                                 sf_spatial,
                                 grid_spatial,
                                 grid_numeric,
  ...
  if (sum(hull_set) != length(observed_in_grid)) {
    ...
    for (i in 1:length(observed_in_grid)) {
      ...
      inner_hull <- compute_hull_inner(inner_observed[, c(1,2)],
...
#' needed in test_ufg_main
#' This function checks which point lies in the spatial boundary of the convex set
check_boundary_input <- function(observed,
                                 observed_in_grid) {
...
#' needed in test_ufg_main
#' This function checks if it could be a ufg-premise based on the boundary values
#' e.g. if there exists an element that is obviously already given by the others
check_ufg_lies_in <- function(is_boundary,
                              grid_numeric,
                              grid_nominal) {
  maybe_ufg <- list()
...
#' This function tests if the subset is a ufg-premise for mixed spatial, numeric,
#' nominal data
test_ufg_main <- function(observed,
                          observed_in_grid,
                          sf_spatial,
                          grid_spatial,
                          grid_numeric,
  ...
  if (length(observed_in_grid) == 3) {
    result <- test_ufg_classic(observed,
                               observed_in_grid,
                               sf_spatial,
                               grid_spatial,
                               grid_numeric,
  ...
  if (length(observed_in_grid) == 4) {
    is_boundary <- check_boundary_input(observed,
                                        observed_in_grid)
    # grid_spatial)
    ...
    if (!is_boundary$is_boundary) {
      ...
      is_not_ufg <- check_ufg_lies_in(is_boundary,
                                      grid_numeric,
                                      grid_nominal)
      if (is_not_ufg) {
        result <- list(is_ufg = FALSE,
...
#' main function that computes the ufg depht for mixed spatial,nominal and numeric
#' data based on an grid and an point pattern
#' @param observed (nx2) the observed values
#' @param observed_in_grid (vector) assignment to the grid
#' @param grid_spatial (Lx2 matrix) the spatial component of the entire grid (L is the number of grids)
...
#' @param lower_bound_ufg (integer) value strictly larger than 1 (lower ufg size bound)
#' @param upper_bound_ufg (integer) value strictly larger than lower_bound_ufg (upper ufg size bound)
compute_ufg_grid <- function(observed,
                             empirical_prob,
                             observed_in_grid,
                             grid_spatial,
                             grid_numeric,
  ...
  for (i in seq(lower_bound_ufg, min(upper_bound_ufg, dim(observed)[1]))) {
    ...
    while (TRUE) {
      ...
      ufg_test <- test_ufg_main(observed_test,
